...
Feature: Monitoring Ratings module - rating lifecycle and queries
  ...
  Background:
    ...
    * def so = callonce read('classpath:common/seed-helper.feature@completedServiceOperation')
    * def rating = karate.merge(testData.newRating, { requestId: so.requestId, technicianId: so.technicianId })
  ...
  Scenario: Update rating returns 200 (empty body)
    ...
    * def updateBody = karate.merge(testData.updateRating, { ratingId: createdId })
    Given path '/ratings'
