...
Feature: SDP Schedules module - technician scheduling
  ...
  Scenario: Get schedules by technician returns the technician's schedules
    ...
    * eval matchEachOrEmpty(response, { id: '#number', technicianId: '#string', day: '##string', startTime: '##string', endTime: '##string' })
  ...
  Scenario: Create schedule returns 200 with the created schedule id (Long)
    ...
    When method POST
    Then status 200
    # POST /schedules returns the created Long id in a raw (text) body,
    # which Karate reads as a string - assert presence, not JSON number type.
