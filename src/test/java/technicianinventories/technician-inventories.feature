...
#   ComponentStockResource         = { componentStockId(UUID), componentId,
#                                      componentName, quantityAvailable,
#                                      alertThreshold, lastUpdated }
#   TechnicianInventoryResource    = { inventoryId(UUID), technicianId,
#                                      stock: ComponentStockResource[] }
#   AddComponentStockResource      = { componentId, quantity, alertThreshold }
#   UpdateComponentStockResource   = { newQuantity, newAlertThreshold }
# ─────────────────────────────────────────────────────────────────
...
Feature: Technician Inventory module - full inventory management
  ...
  Scenario: Add a component to the technician stock
    ...
    And match response ==
      ...
      """
    And match response.stock[*].componentId contains testData.newStock.componentId
  ...
  Scenario: Get a single stock item by technician and component id
    Given path '/technician-inventories/technician', 1, 'stocks', testData.newStock.componentId
    When method GET
    Then status 200
    And match response ==
      """
      ...
      """
    And match response.componentId == testData.newStock.componentId
